...
export function QualitativeCard({ id, label, score, structured, isLive, headerAddon }: QualitativeCardProps) {
  return (
    <section id={id} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden">
      ...
      <div className="p-5">
        {structured ? (
          <AICompanionPanel structured={structured} categoryLabel={label} expandAnalysis={true} />
        ) : isLive ? (
          <PanelSkeleton />
        ) : null}
      </div>
