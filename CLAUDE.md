...
Fourth bounded-modifier signal (2026-06-12): **graph centrality** — `services/graph_centrality.py` is pure NetworkX over the theme-wide graph from `build_theme_graph`. Betweenness + eigenvector are computed and classified **per connected component** (theme graphs are routinely disconnected; global eigenvector squashes smaller components toward 0); ≥10-edge theme gate, ≥5-edge per-component classification gate; hub = top-20%-ceil eigenvector AND degree ≥ 3, broker = top-20% betweenness > 0; ties at the cutoff all qualify. Computed daily per theme by `signal_scheduler.refresh_theme_centrality` (called in `run_daily_refresh` after the X pass; never raises — failures roll back partial writes because the session is shared across the theme loop), persisted via the same `_persist_signal_set` dual-write as `signals` `signal_type="centrality"`. Discovery applies `apply_centrality_modifier` (+3 hub / +2 broker — larger wins, set upstream in `modifier_from_scores`; 48h staleness) after the congress modifier; `CentralitySnapshot` rides the card and renders a Hub/Broker chip in the theme-detail expanded company view. Card serialization (inverted 2026-07-01, PR #62): `CompanySignalCard.to_dict()` in `services/discovery.py` walks `dataclasses.fields` recursively, so a new card field reaches the wire automatically unless explicitly excluded in `_wire_value` (whose `Citation` branch deliberately emits only the 5 keys the frontend `Citation` type declares). `api/discovery.py::_card_to_dict` is now a thin shim over it; inversion pinned by `test_discovery_card_serialization.py` — the old hand-enumerated serializer shipped the congress + centrality chips invisible before this.
...
**Phase A — Prompt integration** (`FILING_EXCERPT_ROUTING` in `graph/deep_dive_routing.py`, consumed by `node_deep_dive` in `graph/nodes.py`):
...
- Dismiss = tombstone: `POST /api/relationships/dismiss` writes a `counterparty_aliases` row with `source="curator_private"` and null canonical fields ("private company — never resolve"); the resolver and the unresolved queue both skip tombstoned norms. `GET /api/relationships/dismissed` lists them; `DELETE /api/relationships/dismiss/{alias_normalized}` undoes one. Route-ordering: `/relationships/dismissed` must stay declared above any `/relationships/{param}` route.
...
- Frontend: two consumers of this endpoint —
  1. `SupplyChainEcosystem` card in the deep-dive dashboard (after Business Quality) calls `getGraph(ticker, { depth: 1 })`. Groups counterparties by type, shows verbatim quotes, bilateral badges, tracker-links, and an **"Explore 2-hop graph →"** deep link to `/filings/graph?root={ticker}`. As of 2026-06-12 it also hosts the self-hiding `ConcentrationFlow` sub-section (suppliers → company → customers ribbons from `magnitude_pct` edges incl. unnamed disclosures; latest-filing dedup kills 10-K/10-Q duplicates; renders only with ≥2 bands; pure layout math in `lib/concentrationFlow.ts`, node-tested; hand-rolled SVG, no d3-sankey dep).
...
- **Theme-wide graph** (sibling builder, 2026-06-12): `build_theme_graph(theme_id, db=db)` selects every named `relationships` row where filer OR resolved counterparty ∈ the theme's `seed_tickers` — separate function from `get_graph`, no BFS/root. Node identity is **ticker-first** (`_theme_node_id`: `ticker:` > `cik:` > `unresolved:<norm>`) so a company appearing as both filer and counterparty unifies into one node; `unnamed` rows excluded in SQL (noise in a density view); self-edges skipped; >300 nodes → `too_dense=true` with empty nodes/edges but real counts; malformed (non-UUID) theme ids short-circuit to None (themes.id is a UUID column — would otherwise 500 at the driver, a hazard the pre-existing theme routes still have). Served by `GET /api/relationships/theme-graph/{theme_id}` (404 on unknown/malformed). Frontend: `/filings/graph/theme?theme=<id>` (URL is the state) — `ThemeGraphView` fetch + rails (pick-a-theme / loading / too-dense / zero-edge), `ForceGraphCanvas` (synchronous d3-force layout ticked in a `useMemo`, React-rendered SVG, d3-zoom pan/zoom with fit-to-bounds on load, pointer-capture drag where `toGraphCoords` uses `getScreenCTM().inverse()` — do NOT revert to manual viewBox math, it skews under `xMidYMid meet`; node clicks are capture-retargeted to the svg so background-clear is guarded by `skipBackgroundClearRef`, not `e.target`), `ThemeGraphSidePanel` (buckets from the already-fetched `edges[]` — no second fetch). Pure adapter + visual encoders in `lib/themeGraph.ts` (node-tested; collapses parallel `(from,to,type)` edges, degree = raw filing-row incidence by design). Cross-links: "Theme map →" on `/filings/graph`, "Root graph view →" back, ⌘K surface.
...
- A saved `ticker_models` row is **not** a prerequisite: preflight reports a missing model as the non-blocking `no_ticker_model` warning (it's in `PreflightStatus.warnings`, not `missing`), and `step_update_refresh` skips the model refresh entirely (`model_skipped=true` on `UpdateRefreshOutput`, `version_before=0`) while still surfacing new EDGAR filings — the earnings-day unblock. An unsaved model draft is still blocking.

### Peer comparison (read this before touching `backend/app/services/peer_comp.py` or `peer_sets.py`)

`services/peer_comp.py` is the single peer-table builder shared by three consumers: `GET /api/peers/{ticker}/comp`, `GET /api/peers/compare?tickers=`, and workspace step 5 (differentiation). Schemas live in `models/peer_comp.py` (re-exported from `workspace_schemas.py` so old persisted `step_outputs` keep validating). 16 metrics from 4 FMP endpoints — note the /stable/ API serves valuation multiples (P/E, EV/EBITDA, P/B, P/FCF, P/S, PEG) from `ratios-ttm`, NOT `key-metrics-ttm` (live-verified 2026-06-09; `_fetch_one` uses a `_first` fallback-key helper; `fcf_margin` exists on neither endpoint and is derived as `p_s ÷ p_fcf`). Peer sets persist in `peer_sets` (ticker PK, JSONB peers) — auto-seeded from resolved `competitor_landscape` tickers ∪ `FMPClient.get_stock_peers`, capped at 8 (manual edits capped at 12). `services/peer_sets.py` functions write WITHOUT committing — callers own the session (API routes commit; `peers_for_ticker` is the read-only curated-first/fallback derivation the workspace step consumes). **Route-ordering footgun:** in `api/peers.py`, `/compare` must stay declared before `/{ticker}` — "compare" parses as a valid ticker (pinned by test). Margin-type metrics pass through `services/metric_guards.py::guard_margin` (shared with `company_snapshot.py`): an arithmetically impossible `grossProfitMarginTTM > 1.0` is nulled (upstream FMP corruption), other margins outside [-5, 1.5] are logged but passed through; the backend emits fraction-form margins and the frontend multiplies by 100 exactly once (pinned by `backend/tests/test_metric_scaling.py`). A third correlated tier (`gross_corruption` + `guard_correlated`): when the impossible-gross trigger fires for a ticker, its EBITDA-derived siblings are nulled too — `ebitdaMarginTTM` + EV/EBITDA in both builders, plus Net Debt/EBITDA on the company overview. Tiny-base growth artifacts beyond ±1000% render "n/m" on both sides (`NM_GROWTH_THRESHOLD` in `peer_comp.py`, mirrored by `frontend/components/company/formatStat.ts`). Frontend surfaces: `/company/[ticker]/peers` tab + `/compare` (URL is the state; no nav link by design).
...
### Material events + insider + congress signals (read this before touching `backend/app/services/material_events_scheduler.py`, `event_classifier.py`, `insider_*.py`, `congress_*.py`, or `api/events.py`)

Daily 06:30 UTC cron (4th job in `main.py::lifespan`) scans the universe (theme seeds ∪ active theses — same derivation as the calendar, via the status board's latest-runs SQL). 8-K side: EDGAR submissions → item-code prefilter (skip non-empty subsets of {7.01, 9.01}; 2.02 kept — guidance lives there; empty items = missing metadata → classify) → Haiku classify (`event_classifier.py`, prefill + `parse_structured_output`, enum-normalized; classification errors are NOT tombstoned so they retry next run) → `Filing` (reuses `edgar_sections_ingest._upsert_filing`) + `material_events` (unique per filing, `dismissed_at` mirrors read-throughs). The 8-K side is fault-isolated: an EDGAR/Haiku failure rolls back and is recorded, but the insider ingest below still runs. Form 4 side: FMP `insider-trading/search` (`limit=100`; the SEC link field is `url`, live-verified 2026-06-10) → `insider_transactions` upsert idempotent on a sha256 `natural_key` over PARSED values (so `1000` vs `1000.0` serialization drift can't mint duplicates); `accession_number`/`sec_link` kept for future raw-EDGAR backfill. `insider_signal.py` is pure (90-day aggregate: open-market P/S only, null-price rows count but don't add value, cluster = ≥2 distinct buyers within 30d) → `signals` row `signal_type="insider"` per (ticker, theme) + `signal_history` dual-write. Discovery applies it as a bounded modifier (`apply_insider_modifier`: +5 cluster / +2 net buying / −3 pronounced selling, 48h staleness via `INSIDER_STALE_HOURS`, clamp [0,100]) — deliberately NOT a 4th weight; `InsiderSnapshot.is_stale` is overloaded (true for fresh-but-zero-modifier too — see its docstring before adding consumers). Near-duplicate 8-Ks (same ticker + event_type within 4 days) are grouped at the API layer by the pure `services/material_events_grouping.py::group_events` (newest member = primary; list responses extend with `group_count`/`group_member_ids`/`group_headlines`, primary fields stay top-level). Status board joins a 14-day undismissed summary per ticker (one query, `_summarize_material_events` — counts groups, materiality still escalates over raw members). `/api/events`: list (filterable, grouped), `{id}/dismiss?group=true` (default — dismisses members within the window of the dismissed primary; subtly different anchor than display grouping, documented in the endpoint), `scan` (202 fire-and-forget; cron is primary). Frontend: `MaterialEventsDrawer` + badge on `/status` (×N badges expandable to member headlines; deep link `/status?expand_events=<ticker>`; seed-only tickers (no board row) render in an Orphan-events section below the board and the deep link auto-opens them), amber attention rows on Today (high materiality, 7d, ×N), insider chip on discovery cards. Congress side (third insider-adjacent signal, 2026-06-11): FMP `/stable/senate-trades` + `house-trades` (live-verified; Capitol Trades' unofficial API is dead) → `congress_transactions` (sha256 natural key over parsed values incl. `owner` — same-day Self/Spouse lines are distinct; coarse amount ranges mean genuinely identical same-day lines collapse, accepted) → `congress_signal.py` pure 90-day aggregate (net value from amount-range MIDPOINTS via `amount_mid`, lower bound for open-ended ranges; cluster = ≥2 distinct politicians within 30d) → `signals` row `signal_type="congress"` + history dual-write → bounded discovery modifier (`apply_congress_modifier`: +3 cluster / +1 net buying / −2 pronounced selling — half-ish of insider's because disclosures lag up to 45 days; shared `_apply_cached_modifier` with the insider path) + congress chip on discovery cards (`CongressSnapshot`, same `is_stale` overloading as insider). Runs inside the same daily scan, fault-isolated AFTER the insider commit so a congress FMP failure rolls back only its own rows.

### Company workspace (read this before touching `backend/app/api/company.py` or `frontend/components/company/`)

Fiscal.ai-inspired per-ticker shell at `/company/[ticker]` (PR #32) with tabs: overview / financials / model / peers / research / theses / transcripts / filings. Backed by the `/api/company` router (`header`, `overview`, `financials`, `transcripts` list/get/summary) over `services/company_snapshot.py` + `services/company_transcripts.py`. The model and filings tabs reuse the existing `/model/[ticker]` and filings surfaces; the Peers tab shares `components/peers/` with `/compare`. "Log trade" on the company header deep-links `/performance?log_trade=TICKER`. Overview stats guard margins via `metric_guards.guard_margin` (shared with peer comp — see Peer comparison) and annualize FMP's cumulative 5Y/10Y growth-per-share fields so the CAGR stat labels are true.

### Prospectus pipeline (read this before touching `backend/app/services/prospectus_*.py` or `frontend/components/prospectus/`)

S-1 / S-1/A analysis pipeline (PR #31), parallel to research/workspace runs. `ProspectusService` (wired as `app.state.prospectus`) runs 4 steps — ingest → relationships → 7 IPO-tuned categories → thesis — with SSE via a replay-on `EventBroker` (same pattern as `WorkspaceService`: late subscribers get a full replay) and per-step session+commit, persisting into `prospectus_reports` (`step_outputs` JSONB; status ingesting | analyzing | completed | failed). Verdicts: `participate | watch_post_lockup | pass`. Key trick: issuers are written into `filings.ticker` / `relationships.ticker` under a **`synthetic_ticker`** (`proposed_ticker` if disclosed, else a slug of `issuer_name`, derived as a `@property` — don't re-normalize at the endpoint boundary), which lets the whole 5-phase EDGAR pipeline work unchanged on pre-IPO issuers. IPO category scores use their own rubric (0-30 disqualifying / 31-55 uncertain / 56-75 typical / 76-100 standout in `graph/prospectus_prompts.py`) — not the equity 40/55/70 tiers.
...
- `lib/api/` — **every** backend call goes through the typed client package: 13 domain modules (`core`, `themes`, `filings`, `pipeline`, `catalysts`, `status`, `workspace`, `peers`, `performance`, `transcripts`, `model`, `prospectus`, `company`) re-exported through `lib/api/index.ts` plus the legacy `lib/api.ts` shim, so existing `@/lib/api` imports keep working — zero call-site changes is the convention. New endpoints/types go in the matching domain module; `BASE` and `apiFetch` live in `core.ts` and stay private to the package (api/* modules import them directly; they are not re-exported). Types mirror backend Pydantic/dataclass shapes; if you change a backend response, update the domain module or TS will silently accept stale shapes at the fetch boundary.
- `components/` — presentational pieces (`Nav`, `ScoreRing`, `SourceBadge`, `VelocityBadge`)
- `components/filings/` — `ThemeFilingsPanel`, `TickerFilingsCard`, `SectionReader` (modal), `CurationPanel` (counterparty resolution queue), `MultiHopGraphView` + `RootHeader` + `HopGroup` + `EdgeRowBody` (the `/filings/graph` page), `ThemeGraphView` + `ForceGraphCanvas` + `ThemeGraphSidePanel` (the `/filings/graph/theme` D3 force map; pure adapter in `lib/themeGraph.ts`)
...
- `components/` — presentational pieces incl. `GlobalCommandPalette` (the global ⌘K dialog: tickers / surfaces / runs / actions / report sections)
...
- **Shared utilities under `components/deep-dive/`:**
  ...
  - `sections.ts` — single registry the SectionNav and the global `GlobalCommandPalette` (root layout) both read, so adding a section only needs one edit to stay in sync. The report-local CommandPalette was retired in B3 phase 2; ⌘K is global now.
...
- **Quant routing** (`QUANT_ROUTING`): deterministic quant fingerprint metric groups (Piotroski F, Altman Z, Beneish M, accruals, FCF conversion, SBC dilution, margin OLS slopes) computed in pure Python by `services/quant_fingerprint.py` from the same 8-quarter FMP statements, attached to `CuratedFinancials.quant_fingerprint` inside `_build_curated_financials` (now in `graph/formatters.py`), rendered into the `{quant_data}` slot by `build_quant_context`. Financial Health / Risk Assessment / Growth & Earnings / Business Quality / Management & Governance. TTM (quarters 0–3) vs prior-TTM (quarters 4–7); every metric independently nullable; Altman/Beneish marked not-applicable for Financial Services. Frontend: `QuantFingerprint.tsx` card (Financials cluster), hidden for runs predating the feature.

The deep dive fetches 11 primary FMP endpoints in parallel: income statement (8Q), balance sheet (8Q), cash flow (8Q), profile, DCF, analyst estimates (8Q), historical price (1Y), earnings transcript, key-metrics-ttm, ratios-ttm, and financial-growth (8Q) — plus a degradable Tier-2 gather (`return_exceptions=True`, each call falls back independently): analyst grades consensus, price-target consensus, ratings snapshot, recent + historical grades, insider Form 4s, and 13F institutional ownership (2026-06-12: `_fetch_institutional` walks back up to 4 quarters via `recent_13f_quarters` — a 13F for the in-progress quarter can't exist — then fetches summary + top holders; renders into `_fmt_fundamentals` as an optional section framed "as of [quarter-end] — filings lag ≥45 days", so every category sees positioning context once; absent data renders nothing, which is what keeps `test_fmt_fundamentals_identity.py` valid. FMP ticker-side endpoints `symbol-positions-summary` + `extract-analytics/holder` are live-verified on the current plan — the old "404 on ticker-side" finding is obsolete; some holder rows ship empty `investorName` → CIK fallback). It also pulls ingested filing sections (if any) via `PipelineService._fetch_filing_sections()`, XBRL facts via `_fetch_edgar_facts()`, and the counterparty graph via `_fetch_counterparty_context()`.
