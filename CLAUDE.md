...
The deep dive fetches 11 primary FMP endpoints in parallel: income statement (8Q), balance sheet (8Q), cash flow (8Q), profile, DCF, analyst estimates (8Q), historical price (1Y), earnings transcript, key-metrics-ttm, ratios-ttm, and financial-growth (8Q) — plus a degradable Tier-2 gather (`return_exceptions=True`, each call falls back independently): analyst grades consensus, price-target consensus, ratings snapshot, recent + historical grades, and insider Form 4s. It also pulls ingested filing sections (if any) via `PipelineService._fetch_filing_sections()`, XBRL facts via `_fetch_edgar_facts()`, and the counterparty graph via `_fetch_counterparty_context()`.
