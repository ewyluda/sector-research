...
- `GET /api/questions`, `/by-ticker`, `POST /api/questions/{id}/dismiss|resolve|retry-auto`, `POST /api/questions/bulk` (ids XOR filter; dismiss/resolve/snooze; `snoozed_until` rows are excluded from open lists and by-ticker rollups until expiry via a shared predicate in `services/questions.py`) (`api/questions.py`) → the `/questions` page (`OpenQuestionsPanel` + `QuestionTickerRollupTable` + URL-state priority/category chips + checkbox bulk bar). Open questions are minted by both pipeline runs and workspace steps; `retry-auto` re-runs the targeted follow-up Haiku.
...
- `lib/api/` — **every** backend call goes through the typed client package: 12 domain modules (`core`, `themes`, `filings`, `pipeline`, `catalysts`, `status`, `workspace`, `performance`, `transcripts`, `model`, `prospectus`, `company`) re-exported through `lib/api/index.ts` plus the legacy `lib/api.ts` shim, so existing `@/lib/api` imports keep working — zero call-site changes is the convention. New endpoints/types go in the matching domain module; `BASE` and `apiFetch` live in `core.ts` and stay private to the package (api/* modules import them directly; they are not re-exported). Types mirror backend Pydantic/dataclass shapes; if you change a backend response, update the domain module or TS will silently accept stale shapes at the fetch boundary.
