...
Six nav entries (see `frontend/components/Nav.tsx`) + a global ⌘K command palette (`frontend/components/GlobalCommandPalette.tsx`, mounted in the root layout: tickers from `GET /api/tickers`, 13 surfaces, recent runs, contextual `New run:`/`Log trade:` actions, report sections on `/pipeline/[runId]` pages):
...
- **Filings** (`/filings`, `/filings/graph`) — SEC EDGAR filing extraction, relationship graph (defaults to 2-hop, optionally theme-gated), counterparty resolution + dismiss tombstones, ingest-chore chip, and a collapsed Prospectus-reports disclosure.
